...
_SYSTEM_PROMPT = (
    "Ты — краткий суммаризатор GitHub-активности проекта Ryazhenka CFW (кастомная прошивка Nintendo Switch). "
    "По списку коммитов и изменённых файлов напиши 1-2 предложения на русском языке: "
    "что именно изменилось (feature/bugfix/refactor/update), какой компонент затронут и "
    "почему это важно для пользователей Switch CFW. "
    "Будь конкретным — называй реальные имена файлов/функций если они информативны. "
    "Не перечисляй SHA-хэши. Не используй markdown. Не пиши «обновление репозитория» без деталей."
)

_RELEASE_SYSTEM_PROMPT = (
    "Ты — краткий суммаризатор релизов GitHub для проекта Ryazhenka CFW (Nintendo Switch homebrew). "
    "По тегу, changelog-описанию и названиям файлов релиза напиши 1-2 предложения на русском: "
    "какие ключевые изменения вошли в релиз и что конкретно улучшится/изменится для пользователей. "
    "Если есть .nro или .zip файл — упомяни это. "
    "Будь конкретным и практичным. Не используй markdown."
)

_PR_SYSTEM_PROMPT = (
    "Ты — краткий суммаризатор GitHub Pull Requests для проекта Ryazhenka CFW. "
    "По номерам, названиям и авторам PR напиши 1 предложение на русском: "
    "что предлагается изменить и какой эффект это окажет на пользователей Switch. "
    "Не используй markdown. "
    "Если PR один — кратко его суть, если несколько — общая тема."
)

_WEEKLY_INSIGHTS_PROMPT = (
    "Ты — аналитик GitHub-активности проекта Ryazhenka CFW (Nintendo Switch custom firmware). "
    "По статистике репозиториев за неделю напиши 2-3 предложения на русском: "
    "краткий вывод об интенсивности разработки, какие компоненты развивались активнее всего, "
    "и есть ли важные тренды для пользователей Switch (новые функции, стабилизация, рефакторинг). "
    "Будь лаконичным, конкретным и аналитическим. Не используй markdown."
)
...
async def summarize_commits(repo_name: str, commits: list) -> Optional[str]:
    ...
    for c in commits[:5]:
        ...
        adds = sum(f.get("additions", 0) for f in c.get("files", []))
        dels = sum(f.get("deletions", 0) for f in c.get("files", []))
        line = f"  [{sha}] {msg}"
        if files:
            line += f" | файлы: {', '.join(files)}"
        if adds or dels:
            line += f" | +{adds}/-{dels} строк"
        lines.append(line)

    prompt = "\n".join(lines) + "\n\nЧто изменилось и зачем (1-2 конкретных предложения на русском):"
